...
<head>
	...
	//使用bootstrap分页插件实现分页功能
        ...
        $("#doSearch").click(function(){
			...
			$.ajax({
				type:"post",
				url:"${pageContext.request.contextPath}/course/docheck.do",
				data:{
					"authorname":$("#author-name-search").val(),
					"coursename":$("#course-name-search").val(),
					"status":$("#course-status-search option:selected").val(),
					"type":$("#course-type-id-search option:selected").val(),   
				...
				success:function(result){
					if(result.responseCode == "2001"){
						location.href="${pageContext.request.contextPath}/course/showSelect.do?pageNo=1";    				
    				}else{
    					$("#errorMsg").tooltip({
    						title:"error",
    						placement:"center",
    				...
    				setTimeout(function(){
    						location.href="${pageContext.request.contextPath}${courseSetsrc}"+pageNo;
    					},1000);
				}
        	...
        	$("#course-type-id").ready(function(){
        		$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath}/courseTypeSet/findP3Name.do",
				dataType:"json",
				success:(function(list){
					for(var i = 0;i<list.length;i++){
						$("#course-type-id").append("<option value="+list[i].id+">"+list[i].typeName+"</option>");
						}
					})
				});
        	}); 
        	...
        	//加载课程类别
        	$("#course-type-id-search").ready(function(){
        		$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath}/courseTypeSet/findP3Name.do",
				dataType:"json",
				success:(function(list){
					for(var i = 0;i<list.length;i++){
						$("#course-type-id-search").append("<option value="+list[i].id+">"+list[i].typeName+"</option>");
						}
					})
				});
        	}); 
                ...
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        <div class="form-group col-xs-6">
                            ...
                            <div class="col-xs-8">
                                <input type="text" id="beginTime" class="form-control" placeholder="请输入开始时间:2017-10-10" />
                            </div>
                        </div>
                        <div class="form-group col-xs-6">
                            <label class="col-xs-3 control-label">结束日期：</label>
                            <div class="col-xs-8">
                                <input type="text" id="endTime" class="form-control" placeholder="请输入结束时间:2017-10-12" />
                            </div>
                        </div>
                    </div>
